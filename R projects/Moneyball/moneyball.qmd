...
trendsal.lostplayers <- ggplot(filter(lost.players,yearID<2003),aes(yearID,round(salary/1e6,2),text=round(salary/1e6,2))) + geom_line(aes(group=playerID,color=playerID)) + geom_point(aes(color=playerID)) + theme_calc() + labs(y='salary ($M)',x='season',caption='salary trend for the lost players up until 2002, after they moved to other teams in 2001')
ggplotly(trendsal.lostplayers,tooltip="text")
```
...
We have a pool of `r pool.players` players. The idea is to compute combinations of 3 players that meet the criteria defined and provide them in order of maximizing the stats `AB`, `OBP` and minimizing the `salary` paid.

For the sake of a relatively quick computing we are going to cut the pool of players down by picking the top 14 sample sorted by descending product `AB x OBP` and ascending `salary`.
...
Organizing the winning combinations into a scatterplot divided in four quadrants, we can gather more insight as to which are the best combinations taking into account that the size of dots represent the `salary.total`.
```{r}
#| label: winning-plot
...
scatterplot.combinations <- ggplot(plot.combinations, aes(AB.combined, OBP.mean,color=quadrant,size=salary.total,text=paste(candidate.players,round(salary.total/1e6,2)))) + geom_vline(xintercept = x_mid) + geom_hline(yintercept = y_mid) + geom_point(alpha=0.45) + theme_calc() + theme(legend.position="none")
ggplotly(scatterplot.combinations,tooltip="text")
```
